...
// InstalledVersion reads the version from the installed event_handler.sh comment header.
// Returns empty string if the script doesn't exist or can't be parsed.
func InstalledVersion(hooksDir string) string {
	hooksDir = expandHome(hooksDir)
	data, err := os.ReadFile(filepath.Join(hooksDir, "event_handler.sh"))
	if err != nil {
		return ""
	}
	// Line 2: "# clorch event handler — generated by clorch <version>"
	for _, line := range strings.SplitN(string(data), "\n", 5) {
		const prefix = "# clorch event handler — generated by clorch "
